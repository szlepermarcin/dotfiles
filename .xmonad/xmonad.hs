...
myManageHook =
  composeAll
    ...
    $ [ [isDialog --> doCenterFloat]
      ...
      , [ className =? c --> doShift (myWorkspaces !! 0) <+> viewShift
            (myWorkspaces !! 0)
        | c <- my1Shifts
        ]
      , [ className =? c --> doShift (myWorkspaces !! 1) <+> viewShift
            (myWorkspaces !! 1)
        | c <- my2Shifts
        ]
      , [ className =? c --> doShift (myWorkspaces !! 2) <+> viewShift
            (myWorkspaces !! 2)
        | c <- my3Shifts
        ]
      , [ className =? c --> doShift (myWorkspaces !! 3) <+> viewShift
            (myWorkspaces !! 3)
        | c <- my4Shifts
        ]
      , [ className =? c --> doShift (myWorkspaces !! 4) <+> viewShift
            (myWorkspaces !! 4)
        | c <- my5Shifts
        ]
      , [ className =? c --> doShift (myWorkspaces !! 5) <+> viewShift
            (myWorkspaces !! 5)
        | c <- my6Shifts
        ]
      , [ className =? c --> doShift (myWorkspaces !! 6) <+> viewShift
            (myWorkspaces !! 6)
        | c <- my7Shifts
        ]
      , [ className =? c --> doShift (myWorkspaces !! 7) <+> viewShift
            (myWorkspaces !! 7)
        | c <- my8Shifts
        ]
      , [ className =? c --> doShift (myWorkspaces !! 8) <+> viewShift
            (myWorkspaces !! 8)
        | c <- my9Shifts
        ]
      , [ className =? c --> doShift (myWorkspaces !! 9) <+> viewShift
            (myWorkspaces !! 9)
        | c <- my10Shifts
        ]
      ]
...
runInTerminal conf@(XConfig { XMonad.modMask = modMask }) command =
  (XMonad.terminal conf) ++ " -e " ++ command
...
myKeys conf@(XConfig { XMonad.modMask = modMask }) =
  ...
  -- FUNCTION KEYS
       , ( (0, xK_F12)
         , spawn $ "xfce4-terminal --drop-down"
         )
  ...
  -- CONTROL + ALT KEYS
       ...
       , ((controlMask .|. mod1Mask, xK_k), io (exitWith ExitSuccess))
       , ((controlMask .|. mod1Mask, xK_l), io (exitWith ExitSuccess))
  ...
  --CONTROL + SHIFT KEYS
       , ( (controlMask .|. shiftMask, xK_Escape)
         , spawn $ "xfce4-taskmanager"
         )
...
main = do
  ...
  xmproc0 <- spawnPipe "xmobar -x 0 $HOME/.xmonad/.xmobarrc " -- xmobar monitor 1
